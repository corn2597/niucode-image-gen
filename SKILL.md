...
On Windows, only runner parameters use one dash: `-StatusFile` and `-TimeoutSeconds`. Use lower-case double-dash image options: `--prompt`, `--output`, `--image`, `--mask`, `--quality`, `--size`, and `--overwrite`. The runner also normalizes the equivalent common PowerShell spellings (`-Prompt`, `-Output`, `-Image`, `-Quality`, `-Size`, and `-Overwrite`) for compatibility before it starts the executable.
