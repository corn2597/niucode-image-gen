...
Pass arguments directly to the executable. Do not create a PowerShell/Bash/CMD wrapper and do not translate long options into shell-script parameters. Never start a second process or poll a file. The executable emits one UTF-8 JSON object on stdout and writes operational logs only to stderr.
